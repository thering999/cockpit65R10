        ...
        <div class="row mb-2  ">
          <div class="col-sm-6">
            <h1 class="m-0" style="padding-top: 10px">
             ตัวชี้วัดติดตามประเมินผล เขตสุขภาพที่ 10
              </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
if($row_data["kpi_type_data"] == '0'){
        ...
        for($i=0;$i<sizeof($rate0);$i++){
          if($rate0[$i] >= $row_data["kpi_tar"]){
           ...
           if($i!=0){ if($p_tar[$i-1]==0&&mysqli_num_rows($resck_tar)>0){$iconcollor[$i] = "class='bg-dark'";}}
...
echo "<table border=0 align=center width=100%><tr><td width = 50%><canvas id=\"myChart2\"></canvas></td><td>
...
<div class=\"card-header\">
      <div class=\"container-fluid\">
        <div class=\"row mb-2  \">
          <div class=\"col-sm-12\">
            <h1 class=\"m-0\" style=\"padding-top: 10px\">
             ตารางแสดงข้อมูลตัวชี้วัด Monitoring เขตสุขภาพที่ 10
              </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
<tr height=80px class=bg-warning  style=font-size:16px;font-weight:bold;>
